...
export default function SecurityAlerts() {
  ...
  useEffect(() => {
    const fetchAlerts = async () => {
      setLoading(true)
      const data = await apiService.getUnknownFaces()
      setAlerts(data.security_alerts || [])
      setStatus(data.alert_status || 'CLEAR')
      setLoading(false)
    }

    fetchAlerts()
    const interval = setInterval(fetchAlerts, 1000) // Refresh every second
  ...
  return (
    <div className={`${bgColor} border-2 rounded-lg p-6 backdrop-blur-sm`}>
      ...
      <div className="mt-4 pt-4 border-t border-gray-700 text-xs text-gray-400">
        <p>Total Unknown Faces: <span className="text-red-300 font-bold">{alerts.length}</span></p>
      </div>
    </div>
  )
}
